 ...
 * INLINED into plugin.js because the desktop plugin loader cannot resolve
 * relative module specifiers ("./teams.js"). Logic is identical; see repo
 * D:/Hermes-Bot-Mode/teams.js for the canonical TDD-tested source.
 */
...
const TEAM_GENERATION_KEY = Symbol('team.generation')
const TEAM_PAGE_ROUTE = '/bot-team'
const TEAM_PAGE_ID = 'team-page'

// ── Stub exports (TASK 0: not implemented) ───────────────────────────────────

function normalizeTeams(items, rosterNames = []) {
 ...
 */
async function projectTeamContext(team, turnId, opts = {}) {
 ...
 */
async function teamPrompt(team, profile, message, turnId, opts = {}) {
  const o = opts || {}
 ...
 */
async function saveTeams(storage, teams) {
 ...
 * @returns {Promise<void>}
 */
async function saveTeamLog(storage, teamId, entry) {
 ...
 */
async function loadTeams(storage) {
 ...
 * @returns {Promise<void>}
 */
async function patchTeamReply(storage, teamId, turnId, member, reply) {
  if (!storage || typeof storage.set !== 'function' || !teamId) return
 ...
 * @returns {Promise<void>}
 */
async function deleteTeam(storage, teamId) {
 ...
 */
async function runTeamFanout(team, message, deps = {}) {
...
const Teams = {
  TEAM_MEMBER_LIMIT,
  TEAM_MAX_COUNT,
  TEAM_CONTEXT_ROW_LIMIT,
  TEAM_CONTEXT_CHAR_LIMIT,
