...
function load() {
  ...
  const source = pluginSource
    ...
    .concat('\nglobalThis.__groups = { groupRoster, knownGroups, saveGroupOrder, moveGroup, $groupOrder };\n')
...
test('source contract: the roster render is sectioned, ordered, and the row menu offers grouping', () => {
  ...
  assert.match(pluginSource, /moveGroup\(section\.group, -1\)/)
  assert.match(pluginSource, /moveGroup\(section\.group, 1\)/)
})
